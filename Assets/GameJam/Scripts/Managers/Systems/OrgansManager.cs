...
public class OrgansManager : MonoBehaviour
{
    ...
    [Header("Hapi Stomach")] [SerializeField, Range(0f, 1f)]  private float hapiScreenWipeMiniBossHpFloor = 0.50f;
    [SerializeField, Range(0f, 1f)] private float hapiScreenWipeBossHpFloor = 0.75f;
    ...
    public float DoubleDamageChance { get; private set; }
    public int ExtraDashCount { get; private set; }
    public bool HasSlowTimeAbility { get; private set; }
    ...
    private void OnOutgoingDamageModify(GameObject attacker, GameObject target, ref float damage)
    {
        if (damage <= 0f) return;
    ...
    private void OnIncomingDamageModify(GameObject attacker, GameObject target, ref float damage)
    {
        if (damage <= 0f) return;
    ...
    public bool TryUseScreenWipe(GameObject playerSource)
    {
        if (!_screenWipeReady) return false;
        if (!_active.TryGetValue(OrgansType.Stomach, out var stomach) || stomach != MiniBossType.Hapi)
            return false;

        _screenWipeReady = false;

        var minions = GameObject.FindGameObjectsWithTag(minionTag);
        for (int i = 0; i < minions.Length; i++)
        {
            var hp = minions[i].GetComponentInParent<Health>();
            if (hp != null) hp.TakeDamage(playerSource, 999999f);
        }

        var miniBosses = GameObject.FindGameObjectsWithTag(miniBossTag);
        for (int i = 0; i < miniBosses.Length; i++)
        {
            var hp = miniBosses[i].GetComponentInParent<Health>();
            if (hp != null) hp.ClampHealthToMaxPercent(hapiScreenWipeMiniBossHpFloor);
        }

        var bosses = GameObject.FindGameObjectsWithTag(bossTag);
        for (int i = 0; i < bosses.Length; i++)
        {
            var hp = bosses[i].GetComponentInParent<Health>();
            if (hp != null) hp.ClampHealthToMaxPercent(hapiScreenWipeBossHpFloor);
        }
